...
The logistic map appears in many mathematical and applied contexts, most notably in population dynamics. Here, $x$ represents the size of a population as a fraction of the environment’s carrying capacity. For example, $x = 0.5$ means the population is at half of its theoretical maximum, while $x = 1$ represents a fully saturated population at its environmental limit.  

The parameter $r$ governs the reproduction rate, determining how the population evolves over time. As we will see, varying $r$ can lead to a surprising range of behaviors, from stable equilibrium to chaotic fluctuations.
"""
...
begin
	...
	function trajplot2(f, iter, xinit, yinit)
	x_traj = [xinit]
	y_traj = [yinit]
	
	for _ in 1:iter
		push!(x_traj, f(x_traj[end]))
		push!(y_traj, f(y_traj[end]))
	end
	
	  plot(1:iter+1, x_traj, 
        label="Trajectory from x₀ = $xinit", 
        marker=:circle, lw=2)

    plot!(1:iter+1, y_traj, 
        label="Trajectory from x₀ = $yinit", 
        marker=:circle, lw=2, linestyle=:dash, 
        legend=:outerbottom)
		xlabel!("Generation")
		ylabel!("Population")
	end
...
md""" Number of iterations: $(@bind iter Slider(1:50, show_value=true)) """

# ╔═╡ 3a89f474-02b5-4a62-90a4-c437df739dfe
let
	#Logistic Map with a=3.8. Chaotic regime.
	f(x) = 4*x*(1-x)
	 # Creates a slider for iteration count
...
### Define parameters and function
	...
	trajplot2(f, iter, xinit, yinit)
	title!(L"$x_{n+1} = 4 x_n (1-x_n)$")
end
